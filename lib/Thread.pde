		...
		for(int i=0; messageIds[i]; i++){
			for (int j = 0; messages[j]; j++){
				if(messages[j].id==messageIds[i]){
					...
					line(prevPosX, posY, posX+(messageIds.length-1-i)*(width-150)/(messageIds.length-1),posY);
					prevPosX = posX+(messageIds.length-1-i)*(width-150)/(messageIds.length-1);
				}
			}
		}
			...
			for (int j = 0; messages[j]; j++){
				if(messages[j].id==messageIds[i]){
					...
					for(int k=0; messages[j].interactions[k]; k++){
						var x = posX+(messageIds.length-1-i)*(width-150)/(messageIds.length-1)+ cos(angle*k+.65)*((messages[j].interactions.length+1)*5+5);
						var y = posY + sin(angle*k+.65)*((messages[j].interactions.length+1)*5+5);
						
						stroke(messages[j].interactions[k].couleur);
						line(posX+(messageIds.length-1-i)*(width-150)/(messageIds.length-1),posY,x,y);
						fill(messages[j].interactions[k].couleur);
						ellipse(x,y,5,5);

						fill(messages[j].couleur);
						noStroke();
						ellipse(posX+(messageIds.length-1-i)*(width-150)/(messageIds.length-1),posY, (messages[j].interactions.length+1)*5,(messages[j].interactions.length+1)*5 );
					...
					if( dist(mouseX, mouseY, posX+(messageIds.length-1-i)*(width-150)/(messageIds.length-1), posY+map(scrollY,0, height-173, 0, -4*height) ) < (messages[j].interactions.length+1)*5 ) {
						pushMatrix();
						translate(0,map(scrollY,0, height-173, 0, +4*height));
						messages[j].showInfoBox();
						popMatrix();
